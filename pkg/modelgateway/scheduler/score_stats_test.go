...
func TestScoreSampleDecisionSkipsClientContextLimit(t *testing.T) {
	decision := scoreSampleDecision(core.AttemptResult{
	...
	require.False(t, decision.CircuitSample)
	require.False(t, decision.RealUserMetric)
	require.Equal(t, core.ErrorCategoryClientRequestError, decision.SkipReason)
	require.Equal(t, core.ErrorCategoryClientRequestError, decision.Reason)
}
