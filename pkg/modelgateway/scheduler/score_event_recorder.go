...
func scoreEventFromAdjustment(result core.AttemptResult, snapshot core.RuntimeSnapshot, adjustment core.ScoreAdjustment) model.ModelGatewayScoreEvent {
	...
	return model.ModelGatewayScoreEvent{
		...
		ResourceID:          strings.TrimSpace(snapshot.Key.ResourceID),
		ResourceType:        strings.TrimSpace(snapshot.Key.ResourceType),
		AccountID:           strings.TrimSpace(snapshot.Key.AccountID),
		AccountType:         strings.TrimSpace(snapshot.Key.AccountType),
		Brand:               strings.TrimSpace(snapshot.Key.Brand),
		Provider:            strings.TrimSpace(snapshot.Key.Provider),
		CredentialIndex:     snapshot.Key.CredentialIndex,
		CredentialSubjectFP: strings.TrimSpace(snapshot.Key.CredentialSubjectFP),
		CredentialFP:        strings.TrimSpace(snapshot.Key.CredentialFP),
		FailureScope:        strings.TrimSpace(result.FailureScope),
		SwitchReason:        strings.TrimSpace(result.SwitchReason),
