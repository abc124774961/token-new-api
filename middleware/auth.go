...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		isRootModelsPath := path == "/models" || strings.HasPrefix(path, "/models/")
		// 检查path包含/v1/messages 或 /v1/models
		if strings.Contains(path, "/v1/messages") || strings.Contains(path, "/v1/models") || isRootModelsPath {
		...
		if !service.GroupInUserUsableGroups(userGroup, tokenGroup) {
			common.SysLog(fmt.Sprintf("token group %s is not listed in user usable groups for user group %s; allowing stored token group", tokenGroup, userGroup))
		}
		if !service.GroupRatioConfiguredOrImplicit(tokenGroup) {
			common.SysLog(fmt.Sprintf("group ratio for stored token group %s is not configured; allowing request with default group ratio fallback", tokenGroup))
		}
