...
func TestGetModelGatewayObservabilitySummaryTreatsPending429AsConcurrencyFlow(t *testing.T) {
	...
	requireTrendRejectReason(t, response.Summary.CircuitErrorTypes, scheduler.CircuitErrorRateLimit, 1)
	requireTrendRejectReason(t, response.Summary.CircuitErrorCounts, scheduler.CircuitErrorRateLimit, 1)
}
...
func TestBuildModelGatewayObservabilitySummaryIncludesUserRequests(t *testing.T) {
	...
	require.Equal(t, int64(4), userRequests.Summary.UserRequests)
	...
	require.Len(t, userRequests.RecentRequests, 5)
	require.Equal(t, "req-user-pending", userRequests.RecentRequests[0].RequestID)
	require.Equal(t, "processing", userRequests.RecentRequests[0].Status)
	require.Equal(t, int64(0), userRequests.RecentRequests[0].CompletedAt)
	require.Equal(t, "req-user-aborted", userRequests.RecentRequests[1].RequestID)
	require.True(t, userRequests.RecentRequests[1].ClientAborted)
	require.Equal(t, "client_aborted", userRequests.RecentRequests[1].Status)
	require.Equal(t, "req-user-other", userRequests.RecentRequests[2].RequestID)
	require.Equal(t, "default", userRequests.RecentRequests[2].ActualGroup)
	require.Equal(t, 1.0, userRequests.RecentRequests[2].ActualGroupRatio)
	require.Equal(t, model.ModelGatewayUserRequestErrorUpstream, userRequests.RecentRequests[3].FinalErrorCategory)
	require.Empty(t, userRequests.RecentRequests[2].FinalErrorCategory)
	require.Equal(t, 21, userRequests.RecentRequests[4].FinalChannelID)
	require.Equal(t, "healthy-channel", userRequests.RecentRequests[4].FinalChannelName)
	require.Equal(t, 1001, userRequests.RecentRequests[4].UserID)
	require.Equal(t, "request_user", userRequests.RecentRequests[4].Username)
	require.Equal(t, "vip", userRequests.RecentRequests[4].ActualGroup)
	require.Equal(t, 0.8, userRequests.RecentRequests[4].ActualGroupRatio)
	require.NotNil(t, userRequests.RecentRequests[4].Billing)
	require.Equal(t, 1001, userRequests.RecentRequests[4].Billing.UserID)
	require.Equal(t, "request_user", userRequests.RecentRequests[4].Billing.Username)
	require.Equal(t, 1588, userRequests.RecentRequests[4].Billing.Quota)
	require.Equal(t, 1200, userRequests.RecentRequests[4].Billing.PromptTokens)
	require.Equal(t, 320, userRequests.RecentRequests[4].Billing.CompletionTokens)
	require.Equal(t, 1520, userRequests.RecentRequests[4].Billing.TotalTokens)
	require.Equal(t, 1.25, userRequests.RecentRequests[4].Billing.ModelRatio)
	require.Equal(t, 0.8, userRequests.RecentRequests[4].Billing.GroupRatio)
	require.Equal(t, 2.0, userRequests.RecentRequests[4].Billing.CompletionRatio)
	require.Equal(t, 100, userRequests.RecentRequests[4].Billing.CacheTokens)
	require.Equal(t, 1200, userRequests.RecentRequests[4].Billing.SubscriptionConsumed)
	require.Equal(t, 388, userRequests.RecentRequests[4].Billing.WalletQuotaDeducted)
	require.Equal(t, 1, userRequests.RecentRequests[4].Billing.WebSearchCallCount)
	require.Equal(t, "pending", userRequests.RecentRequests[4].UpstreamCostSource)
	require.Equal(t, "pending", userRequests.RecentRequests[4].UpstreamCostAccuracy)
...
func TestBuildModelGatewayObservabilitySummaryIncludesHealthProbeUserRequestsByDefault(t *testing.T) {
	...
	require.Equal(t, int64(0), trend.UserRequests)
...
func TestModelGatewayObservabilitySummaryExposesDynamicBillingOverview(t *testing.T) {
	...
	require.Equal(t, "codex-pro", group.DisplayGroup)
...
func TestModelGatewayObservabilitySummary7dOverviewSkipsSamplesBeforeDynamicBillingEnabledAt(t *testing.T) {
	...
	require.Equal(t, "codex-plus", group.DisplayGroup)
...
func TestClearModelGatewayClientEmptyOutputAvoidanceClearsSessionScope(t *testing.T) {
	setupModelGatewayReplayControllerTestDB(t)
	modelgatewayintegration.ResetDefaultRuntimeObservabilityDeps()
	t.Cleanup(modelgatewayintegration.ResetDefaultRuntimeObservabilityDeps)
	...
	runtimeDeps := modelgatewayintegration.DefaultRuntimeObservabilityDeps()
	require.NotNil(t, runtimeDeps)
	require.NotNil(t, runtimeDeps.ClientEmptyOutputSwitchTracker)

	now := time.Date(2026, time.June, 10, 12, 0, 0, 0, time.UTC)
	tracker := runtimeDeps.ClientEmptyOutputSwitchTracker.WithNowForTest(func() time.Time { return now })
	sessionKey := "session:test:client-empty-output"
	runtimeKey := core.RuntimeKey{
		RequestedModel: "gpt-5.5",
		UpstreamModel:  "gpt-5.5",
		ChannelID:      909,
	...
	for _, offset := range []time.Duration{-20 * time.Second, -10 * time.Second, -5 * time.Second} {
		tracker.Report(context.Background(), core.AttemptResult{
			ClientSessionKey: sessionKey,
			Key:              runtimeKey,
			ChannelID:        runtimeKey.ChannelID,
			ModelName:        runtimeKey.RequestedModel,
			SelectedGroup:    runtimeKey.Group,
			EndpointType:     runtimeKey.EndpointType,
			Success:          true,
			EmptyOutput:      true,
			ObservedAt:       now.Add(offset),
		})
	}
	dispatchReq := core.DispatchRequest{
		ClientSessionKey: sessionKey,
		RequestedGroup:   runtimeKey.Group,
		UserGroup:        runtimeKey.Group,
		ModelName:        runtimeKey.RequestedModel,
		EndpointType:     runtimeKey.EndpointType,
	}
	candidate := core.Candidate{
		Channel:       &model.Channel{Id: runtimeKey.ChannelID, Name: "empty-output"},
		Group:         runtimeKey.Group,
		UpstreamModel: runtimeKey.UpstreamModel,
		RuntimeKey:    runtimeKey,
	}
	snapshot := core.RuntimeSnapshot{Key: runtimeKey}
	_, active := tracker.AvoidanceInfo(dispatchReq, candidate, snapshot)
	require.True(t, active)

	router := gin.New()
	router.POST("/api/model_gateway/observability/runtime/clear_client_empty_output_avoidance", ClearModelGatewayClientEmptyOutputAvoidance)
	body := `{"session_key":"session:test:client-empty-output","channel_id":909,"requested_model":"gpt-5.5","group":"pro","endpoint_type":"openai"}`
	resp := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/api/model_gateway/observability/runtime/clear_client_empty_output_avoidance", strings.NewReader(body))
	router.ServeHTTP(resp, req)

	var payload struct {
		Success bool                                                `json:"success"`
		Message string                                              `json:"message"`
		Data    ModelGatewayClientEmptyOutputAvoidanceClearResponse `json:"data"`
	}
	require.NoError(t, common.Unmarshal(resp.Body.Bytes(), &payload), resp.Body.String())
	require.True(t, payload.Success, payload.Message)
	require.True(t, payload.Data.Cleared)
	require.Equal(t, sessionKey, payload.Data.Scope.SessionKey)
	_, active = tracker.AvoidanceInfo(dispatchReq, candidate, snapshot)
...
func TestGetModelGatewayHealthCheckQueueReturnsPendingReasons(t *testing.T) {
	useModelGatewayHealthCheckSnapshotOnly(t)
	now := time.Unix(1710000000, 0)
	restoreSetting := scheduler_setting.SetSettingForTest(scheduler_setting.SchedulerSetting{
...
func TestGetModelGatewayHealthCheckQueueShowsOverloadRecovery(t *testing.T) {
	useModelGatewayHealthCheckSnapshotOnly(t)
	now := common.GetTimestamp()
	restoreSetting := scheduler_setting.SetSettingForTest(scheduler_setting.SchedulerSetting{
		ProbeEnabled:                        true,
	...
	resp := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/api/model_gateway/observability/health-check/queue?queue_type=recovery&model=gpt-5.5&group=vip", nil)
	router.ServeHTTP(resp, req)
	...
	require.Len(t, payload.Data.Items, 1)
	item := payload.Data.Items[0]
	require.Equal(t, 821, item.ChannelID)
	require.Equal(t, "recovery", item.QueueType)
	require.Empty(t, item.ProbeSkipReason)
	require.InDelta(t, common.GetTimestamp(), item.NextProbeAt, 2)
	require.Equal(t, int64(0), item.NextProbeRemainingSeconds)
	require.True(t, modelGatewayHealthCheckTestReasonsContain(item.Reasons, service.ChannelOverloadRecoveryReason))
}
...
func useModelGatewayHealthCheckSnapshotOnly(t *testing.T) {
	t.Helper()
	oldDB := model.DB
	oldMemoryCache := common.MemoryCacheEnabled
	model.DB = nil
