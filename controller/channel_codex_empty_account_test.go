...
func TestAddEmptyCodexAccountPoolChannelAutoDisablesUntilImport(t *testing.T) {
	...
	require.False(t, importPayload.Data.Operation.ChannelRestored)
	require.Equal(t, 1, importPayload.Data.Total)
	require.Equal(t, 0, importPayload.Data.Enabled)
	require.Equal(t, 1, importPayload.Data.Disabled)
	...
	require.Equal(t, channelAccountManualDisabledReason, channel.GetOtherInfo()["status_reason"])
	require.False(t, channel.ChannelInfo.IsMultiKey)
	require.Equal(t, 1, channel.ChannelInfo.MultiKeySize)
	require.NoError(t, db.First(&ability, "channel_id = ?", channelID).Error)
	require.False(t, ability.Enabled)

	enableAccountBody, err := common.Marshal(gin.H{"enabled": true})
	require.NoError(t, err)
	enableAccountRecorder := httptest.NewRecorder()
	router.ServeHTTP(enableAccountRecorder, httptest.NewRequest(http.MethodPost, "/api/channel/"+strconv.Itoa(channelID)+"/accounts/0/status", bytes.NewReader(enableAccountBody)))
	enableAccountPayload := decodeChannelAccountsResponse(t, enableAccountRecorder)
	require.True(t, enableAccountPayload.Success, enableAccountRecorder.Body.String())
	require.True(t, enableAccountPayload.Data.Operation.ChannelRestored)
	require.Equal(t, 1, enableAccountPayload.Data.Enabled)
	require.Equal(t, 0, enableAccountPayload.Data.Disabled)
